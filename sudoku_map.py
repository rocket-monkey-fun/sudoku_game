...
def func_start_creating_maps():
    random_numbers_first_row = random.sample(list_numbers, k = 9)
    list_valid_map.append(random_numbers_first_row)

    while len(list_valid_map) != 9:
        current_time = time.time()
        elapsed_time = current_time - start_time[0]
        if elapsed_time > 8 or stop_creating[0] == True:
            break
        else:
            random_numbers = random.sample(list_numbers, k = 9)

            print(len(list_valid_map))

            while len(list_valid_map) == 1:
                ...
                if iteration_1((6, 9), (0, 1), (6, 9), random_numbers) == False:
                    ...
                    list_valid_map.append(random_numbers)
                    break

            while len(list_valid_map) == 2:
    ...
    if len(list_valid_map) != 9 and stop_creating[0] == False:
        func_start_creating_maps_button()

    if stop_creating[0] == False:
        open_new_map = open("valid_maps.txt", "a")
        open_new_map.write("\n")
        open_new_map.write(f"{list_valid_map}")
        open_new_map.close()

        list_valid_map.clear()
        func_start_creating_maps_button()
    ...
    for i in range(range_i[0], range_i[1]):
        for j in range(range_j[0], range_j[1]):
            for k in range(range_k[0], range_k[1]):
                if random_numbers[i] != list_valid_map[j][k]:
                    stage.append(1)
                    if len(stage) == 9:
                        stage.clear()
                else:
...
def iteration_2(range_i, range_j, random_numbers):
    for i in range(range_i[0], range_i[1]):
        for j in range(range_j[0], range_j[1]):
            if random_numbers[i] != list_valid_map[j][i]:
                stage.append(1)
                if len(stage) == 9:
                    stage.clear()
            else:
